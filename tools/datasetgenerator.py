...
def get_first_non_empty(row):
    import pandas as pd
    for col in ['pathJADNifti', 'pathKWLNifti', 'pathMarjoNifti','pathJADMLNifti', 'pathVerifiedMLNifti']:
        if pd.notna(row[col]) and row[col] != '':
            return row[col]
    return None  # Default to empty if all are empty
    
def datasetgenerator_bergen(df, require):
    """
    ...
    fntab = [
        ...
        'pathJADNifti', 'pathKWLNifti', 'pathMarjoNifti','pathVerifiedMLNifti', 'pathJADMLNifti'
    ]
    df = df[fntab]
...
def datasetgenerator_mont(df, require):
    """
    Generate a filtered and annotated dataset from a given DataFrame.
    ...
    # (i.e., dropping any columns not in fntab)
    fntab = [
        'subj', 'pathT1KDicom', 'pathT2Dicom', 'pathADCDicom', 'pathNLNifti'        
    ]
    df = df[fntab]
    ...
    #    manual mask path columns (JAD, KWL, VerifiedML, or JADML).
    #    The first non-null path in that priority order is used.
    df['pathmask'] = df["pathNLNifti"].copy() #df.apply(get_first_non_empty, axis=1)
...
def datasetgenerator_hong_kong(df, require):
    raise NotImplementedError(
        "Hong-Kong dataset er registrert som placeholder, men datasetgenerator_hong_kong() "
        "må oppdateres med riktige kolonner, maskelogikk og filtrering før det kan brukes."
    )
