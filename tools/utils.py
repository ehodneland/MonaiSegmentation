...
def _parse_hf_model_ref(model_ref, revision=None):
    model_ref = str(model_ref).strip()

    if model_ref.startswith("hf://"):
        return model_ref[len("hf://"):].strip("/"), revision

    parsed = urlparse(model_ref)
    if parsed.netloc == "huggingface.co":
        parts = [p for p in parsed.path.split("/") if p]
        if len(parts) < 2:
            raise ValueError(f"Could not parse Hugging Face repo id from: {model_ref}")
...
def _looks_like_hf_repo_id(model_ref):
    ...
    if model_ref.startswith((".", "~")):
        return False
    if model_ref.startswith(("hf://", "https://huggingface.co/", "http://huggingface.co/")):
        return True

    parts = [p for p in model_ref.split("/") if p]
    return len(parts) == 2 and not any(part in {".", ".."} for part in parts)


def resolve_model_dir_from_hf(model_ref, revision=None, cache_dir=None, token=None):
    """
    Return a local model directory.

    model_ref can be a normal local path, a Hugging Face repo id such as
    "user/repo", an "hf://user/repo" reference, or a huggingface.co URL.
    """
    model_ref = os.path.expanduser(str(model_ref))
    ...
    try:
        ...
        raise ImportError(
            "huggingface_hub is required to read model_dir from Hugging Face Hub. "
            "Install it with: pip install huggingface_hub"
        ) from exc

    repo_id, revision = _parse_hf_model_ref(model_ref, revision=revision)
    print(f"Laster modell fra Hugging Face Hub: {repo_id}")
    if revision:
    ...
    try:
        ...
        if exc.__class__.__name__ in {"RepositoryNotFoundError", "GatedRepoError"}:
            raise RuntimeError(
                f"Could not download Hugging Face model repo '{repo_id}'.\n\n"
                "Check that:\n"
                f"  1. The repo exists at https://huggingface.co/{repo_id}\n"
                "  2. The repo id is spelled exactly right, including capitalization.\n"
                "  3. If the repo is private, your token has read access.\n"
                "  4. The token file contains only the hf_... token, no extra text.\n\n"
                "You can test the same token with:\n"
                "  hf auth whoami --token $(cat hf_token)\n"
            ) from exc
        raise
...
def save_training_artifacts(
    ...
    def with_split(df, split):
        out = df.copy()
        out["split"] = split
        return out

    case_table_parts = [
        with_split(train_df, "train"),
        with_split(val_df, "val"),
    ]
    if test_df is not None:
        case_table_parts.append(with_split(test_df, "test"))
    case_table = pd.concat(case_table_parts, ignore_index=True)

    front_cols = [
        c for c in ["subj", "source", "split", "imgpath", "pathmask"]
        if c in case_table.columns
    ]
    other_cols = [c for c in case_table.columns if c not in front_cols]
    case_table = case_table[front_cols + other_cols]
...
#    pd.DataFrame([settings]).to_csv(settings_path, index=False, sep=";")
    ...
    model_meta = {
        ...
        "case_table_file": "case_table.xlsx",
        "case_table_csv": "case_table.csv",
    }
    ...
    # ======================================================
    # 3) Lagre samlet case-tabell med split-tag
    # ======================================================
    case_table_xlsx = os.path.join(save_dir, "case_table.xlsx")
    case_table_csv = os.path.join(save_dir, "case_table.csv")
    with pd.ExcelWriter(case_table_xlsx, engine="openpyxl") as writer:
        case_table.to_excel(writer, sheet_name="cases", index=False)
    case_table.to_csv(case_table_csv, index=False, sep=";")

    print(f"Case-tabell lagret til:")
    print(f"  - {case_table_xlsx}")
    print(f"  - {case_table_csv}")

    return meta_path



def collate_multichannel_batch(batch, n_channels):
    """Stack a TorchIO batch into image and mask tensors.

    This function must live at module scope so DataLoader workers can pickle it
    when Python uses the ``forkserver`` or ``spawn`` start method.
    """
    xs, ys = [], []

    for item in batch:
        channels = [item[f"ch{i}"][tio.DATA] for i in range(n_channels)]
        x = torch.cat(channels, dim=0)          # (C, Z, Y, X)
        y = item["mask"][tio.DATA].float()     # (1, Z, Y, X)
...
def get_dataloader(
    ...
    return DataLoader(
        ...
        pin_memory=True,
        collate_fn=partial(collate_multichannel_batch, n_channels=n_channels),
...
def nnunet_resenc_l_model(in_channels):
    ...
    try:
        ...
        raise ImportError(
            ...
            "Installer arkitekturpakken i det aktive treningsmiljøet, for "
            "eksempel: python -m pip install dynamic-network-architectures timm"
        ) from exc
...
def swinunetr_model(
    ...
    """
    Oppretter en 3D SwinUNETR fra MONAI.

    Dette er en praktisk foundation-/pretraining-vennlig modell i samme
    input/output-format som de andre segmenteringsmodellene her.
    """
    # SwinUNETR has five downsampling levels, so every spatial dimension must
    # be divisible by 2**5. MONAI >= 1.5 validates this during ``forward``;
    # older versions received the fixed size through the constructor.
    spatial_size = (img_size,) * 3 if isinstance(img_size, int) else tuple(img_size)
    if len(spatial_size) != 3 or any(size % 32 != 0 for size in spatial_size):
        raise ValueError(
            "SwinUNETR krever at alle romlige dimensjoner er delelige med 32; "
            f"fikk img_size={img_size}."
        )
...
def resolve_pretrained_checkpoint_nnUNet(pretrained, checkpoint_name="checkpoint_best.pth"):
    ...
    if os.path.isfile(settings_path):
        with open(settings_path) as f:
            meta = json.load(f)
        local_result_checkpoint = os.path.join(
            pretrained,
            "nnUNet_work",
            "nnUNet_results",
            meta["nnunet_dataset_name"],
        ...
        if os.path.isfile(local_result_checkpoint):
            return local_result_checkpoint
