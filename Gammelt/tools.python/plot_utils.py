...
def scroll_through_fused_channels(img, mask, transpose_order=None, alpha_init=0.4):
    """
    Interaktiv visualisering av 3D/4D volum med fusjonert maske.
    Basebildet er alltid skarpt (ett imshow), masken blends inn/ut i RGB-arrayet.
    """
...
def scroll_through_slices(img, mask, channel=0, num=0, transpose_order=None):
    ...
    - mask: Maskevolum med samme form som bilde
...
def show_mask_over_time(im, timeline, mask, slice_index, alpha=0.5, red_cmap='Reds', savefig=None):
    """
    Viser ett spesifikt snitt av en 4D-bildeserie over alle tidspunkter.

    Parametere:
    - im (numpy.ndarray): 4D-bilde (T, D, H, W) der T = tid.
    - timeline (array-like): Liste over tidssteg (samme lengde som T).
    - mask (numpy.ndarray): 3D-maske (D, H, W).
    - slice_index (int): Indeks for snittet som skal vises (akse 1) eller 'largest_mask_slice'
    - alpha (float): Gjennomsiktighet for mask-overlay.
    - red_cmap (str): Navn på colormap for mask (default 'Reds').
    - savefig (str eller None): Hvis angitt, lagrer til denne stien.
    """
    if slice_index == "largest_mask":
        slice_index = largest_mask_slice(mask)

    assert im.ndim == 4, f"Bildevolumet må være 4D (T, D, H, W), men fikk {im.shape}"
    assert mask.ndim == 3, f"Masken må være 3D (D, H, W), men fikk {mask.shape}"
    assert 0 <= slice_index < im.shape[1], f"slice_index {slice_index} er utenfor rekkevidde"
...
def plot_images_and_mask(
    ...
    """
    Plot bilder med rødt mask-overlegg + en unik maske i svart/hvitt.
    ...
    - slices_to_plot_mode: 'all', 'largest_mask', eller 'mask_slices'
    ...
    if slices_to_plot_mode == "all":
        ...
        raise ValueError(f"Ugyldig slices_to_plot_mode: {slices_to_plot_mode}")

    if len(slices_to_plot) == 0:
        print("Ingen slices å vise - masken er tom.")
        return

    num_rows = len(slices_to_plot)
    num_cols = nim + 1  # bilder med overlay + én maske
...
def plot_images_and_masks(
    ...
    """
    Plot bilder med rødt mask-overlegg + separat maske (for hver kombinasjon).

    Parametere:
    - imagelist: Liste med 3D-bilder [(D, H, W), ...] eller 4D-array (N, D, H, W)
    - masklist: Liste med 3D-masker [(D, H, W), ...] eller én 3D-maske
    - names: Navn på bildene
    - subj: Subjekt-ID
    - pathsave: Sti for lagring (PDF/png), None for visning
    - slices_to_plot_mode: 'all', 'largest_mask', eller 'mask_slices'
    ...
    if len(masklist) == 1 and len(imagelist) > 1:
        masklist = masklist * len(imagelist)  # samme maske for alle

    assert len(imagelist) == len(masklist), "imagelist og masklist må ha samme lengde."
    ...
    if slices_to_plot_mode == "all":
        ...
        # Finn slice med største maske (bruk første maske)
    ...
    if len(slices_to_plot) == 0:
        print("Ingen slices å vise - maskene er tomme.")
        return

    num_rows = len(slices_to_plot)
    num_cols = nim * 2  # hver bilde+maske gir 2 kolonner
