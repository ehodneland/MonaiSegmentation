...
from tools.utils_finetuning import (
...
def setup_training_log(save_dir):
    log_path = os.path.join(save_dir, "training.log")
    log_file = open(log_path, "w", buffering=1)
    compact_log = CompactLogStream(log_file)

    sys.stdout = TeeStream(sys.__stdout__, compact_log)
    sys.stderr = TeeStream(sys.__stderr__, compact_log)

    print("=" * 80)
    print(f"Training log started: {datetime.datetime.now().isoformat(timespec='seconds')}")
    print(f"Log file: {log_path}")
    print("Command:", " ".join(shlex.quote(arg) for arg in sys.argv))
    print("=" * 80)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="UNet",
        choices=["UNet", "nnUNet", "SSM", "SwinUNETR"],
        help=(
            "Modellarkitektur: MONAI UNet, nnU-Net v2-inspirert ResEnc L, "
            "SSM/Mamba-inspirert 3D-modell, eller MONAI SwinUNETR."
        ),
    )
    ...
    parser.add_argument(
        ...
        choices=[12, 24, 48],
        help=(
            "Feature size for SwinUNETR. MONAI BraTS21 pretrained checkpoints "
            "bruker 48; small/BTCV bruker 24."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Eksisterende modellmappe, checkpoint-fil eller Hugging Face repo/URL "
            "for finetuning."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "For SwinUNETR: head=out-lag; decoder=decoder+out; "
...
def pretrained_source_name(pretrained_ref, resolved_pretrained_dir):
    if os.path.isfile(resolved_pretrained_dir):
        return os.path.basename(os.path.dirname(resolved_pretrained_dir))

    settings_path = os.path.join(resolved_pretrained_dir, "settings.json")
    ...
    return os.path.basename(str(pretrained_ref).rstrip("/"))
...
def main():
    ...
    if args.pretrained:
        ...
        pretrained_name = pretrained_source_name(args.pretrained, pretrained_dir)

    if args.pretrained:
        basename = build_finetuned_model_name(
            pretrained_dir,
            datasets,
            source_model_name=pretrained_name,
        )
    else:
        basename = (
    ...
    setup_training_log(save_dir)

    if args.pretrained:
        print(f"Finetuning fra pretrained modell: {args.pretrained}")
